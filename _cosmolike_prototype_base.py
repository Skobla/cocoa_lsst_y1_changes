...
class _cosmolike_prototype_base(DataSetLikelihood):
  ...
  def initialize(self, probe):
    ...
    self.do_cache_cosmo = np.zeros(2) # previously np.zeros(2)
    
    # ------------------------------------------------------------------------
    #changer = self.provider.get_param("changer")
    #if changer == 1:
  ...
  def set_cosmo_related(self):
    ...
    if changer == 1:
        raise LoggedError(self.log, "EE2 with kz modification not implemented, the z range is limited", changer)
        ...
        # k loop: note the difference in z and k here
            ...
            z_add_smooth = gaussian_filter1d(z_add, sigma=sigma_z)
            k_add_smooth = gaussian_filter1d(k_add, sigma=sigma_k)
            addition_array = np.add.outer(z_add_smooth, k_add_smooth)
        elif changer == 3:
            for i in range(len(self.BETA_LIST)):
                for j in range(len(self.ALPHA_LIST)):
                    if self.BETA_LIST[i] != 1 and self.ALPHA_LIST[j] != 1:
                        z_mask = (z_indices == i)
                        k_mask = (k_indices == j)
                        cross_section_mask = np.outer(z_mask, k_mask)
                        addition_array[cross_section_mask] = np.log(self.BETA_LIST[i]) + np.log(self.ALPHA_LIST[j])
        np.save("lnPNL_DES_test1", lnPNL)
        np.save("lnPL_DES_test1", lnPL)
      
        lnPL += addition_array
        lnPNL += addition_array
        np.save("lnPNL_DES_test2", lnPNL)
        np.save("lnPL_DES_test2", lnPL)
        ...
        print("saving test")
        np.save("lnPNL_DES_optimal_changed2", lnPNL)
        np.save("lnPL_DES_optimal_changed2", lnPL)
        ## KZ END

    else:
      raise LoggedError(self.log, "changer = %d is an invalid option", changer)
