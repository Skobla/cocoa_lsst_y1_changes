...
class _cosmolike_prototype_base(DataSetLikelihood):
  ...
  def initialize(self, probe):
    ...
    self.entered_k_list = list()
    self.entered_z_list = list()
    self.entered_alpha_list = list()
    self.entered_beta_list = list()
  ...
  def set_cosmo_related(self):
    ...
    # SK START
    #If manually entering and not using yaml file to add the k, z parameters
    #self.entered_k_list += [1e-4, 1e-3, 1e-2, 1e-1, 1]
    #self.entered_z_list += [0, 0.2, 0.4, 0.8, 1.6]
    #self.entered_alpha_list += [1.1, 0.9, 1.1, 0.9, 1.1]
    #self.entered_beta_list += [1, 1, 1, 1, 1]

    #If using yaml file to add the k, z parameters then:
    ...
    for i in range(int(kbins)):
        ...
        self.entered_alpha_list.append(adder_value)
    ...
    for i in range(int(zbins)):
        ...
        self.entered_beta_list.append(adder_value)

    self.entered_alpha_list = np.log(self.entered_alpha_list)
    self.entered_beta_list = np.log(self.entered_beta_list)
    
    #Use functions in the array addition if functional form desired
    def lnalpha_of_k(self,x):
        if x == 0:
            alpha_list = np.ones(len(self.entered_k_list)) 
        else:
            assert len(self.entered_k_list) == len(self.entered_alpha_list)
            alpha_list = self.entered_alpha_list
            #insert functional form here if desired and log the list if function doesn't do so
        return alpha_list
    
    def lnbeta_of_z(self, x):
        if x==0:
            beta_list = np.ones(len(self.entered_z_list))
        else:
            assert len(self.entered_z_list) == len(self.entered_beta_list)
            beta_list = self.entered_beta_list
            #insert functional form here if desired and log the list if function doesn't do so
        return beta_list
    # SK END
    ...
    #reverse_z_list_temp = self.entered_z_list #SK
    #reverse_z_list = list(reversed(reverse_z_list_temp)) # SK
    #reverse_k_list_temp = self.entered_k_list #SK
    #reverse_k_list = list(reversed(reverse_k_list_temp)) #SK
    ...
    for i in range(self.len_z_interp_2D):
      ...
      #for j in reverse_z_list: #SK
          #counter = 0 #SK
          #if self.z_interp_2D[i] >= j: #SK
              #if counter == 0: #SK
                #lnPNL[i::self.len_z_interp_2D] += lnbeta_of_z(self, 0)[int(-1 - j)] #SK
                #lnPL[i::self.len_z_interp_2D] += lnbeta_of_z(self, 0)[int(-1 - j)] #SK
                #counter += 1 #SK
    #for i in range(self.len_k_interp_2D): #SK
      #for k in reverse_k_list: #SK
          #counter = 0
          #if self.k_interp_2D[i] >= k: #SK
              #if counter == 0: #SK
                  #lnPNL[i::self.len_k_interp_2D] += lnalpha_of_k(self, [1.1,0.9,1.1,0.9,1.1])[int(-1 - k)] #SK
                  #lnPL[i::self.len_k_interp_2D] += lnalpha_of_k(self, [1.1,0.9,1.1,0.9,1.1])[int(-1 - k)] #SK
                  #counter += 1 #SK
    ...
    #SK START
    #Adding on the values by k and z bin as created above
    lnPL = lnPL.reshape(-1, 150).T
    lnPNL = lnPNL.reshape(-1, 150).T
    ...
    z_indices = np.clip(z_indices, 0, len(self.entered_beta_list) - 1)
    k_indices = np.clip(k_indices, 0, len(self.entered_alpha_list) - 1)

    z_add_array = self.entered_beta_list[z_indices][:, np.newaxis]
    k_add_array = self.entered_alpha_list[k_indices][np.newaxis, :]

    addition_array = z_add_array + k_add_array
    ...
    #file_to_save_ps = 'chi_ps_data/ps' + bin_name + '.npy'
    #np.save(file_to_save_ps, lnPL) #SK
    #np.save('chi_ps_data/psORIGINAL.npy', lnPL)

    # ------------------------------------------------------------------------
    # SK START
    
    # assuming we are taking in k in h/Mpc
    #self.entered_k_list += [10**-4, 10**-3, 10**-2, 1, 10]
    ...
    print('chi:', chi)
    #print('chi_len:' chi.shape)
    #file_to_save_chi = 'chi_ps_data/chi' + bin_name + '.npy'
    #np.save(file_to_save_chi, chi)
    #np.save('chi_ps_data/chiORIGINAL.npy', chi)
  ...
  def generate_baryonic_PCA(self, **params_values):
    ...
    i.init_baryons_contamination(False,"")
