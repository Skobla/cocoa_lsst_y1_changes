...
class _cosmolike_prototype_base(DataSetLikelihood):
  ...
  def set_cosmo_related(self):
    ...
    for i in range(1200-1):
        if self.k_interp_2D[i] >= self.k_interp_2D[i+1]:
            diff = self.k_interp_2D[i] - self.k_interp_2D[i+1]
            self.k_interp_2D[i] -= 1.0001*diff
    ...
    #np.save('Fiducial_Knewtry.npy', lnPL) #SK
